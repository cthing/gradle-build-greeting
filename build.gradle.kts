...
gradlePlugin {
    website = "https://github.com/cthing/gradle-build-greeting"
    vcsUrl = "https://github.com/cthing/gradle-build-greeting"

    plugins {
        create("buildGreetingPlugin", Action {
            id = "org.cthing.build-greeting"
            displayName = "Build greeting"
            description = "A Gradle plugin that shows build environment information at the start of a Gradle run."
            tags = listOf("build", "greeting", "environment")
            implementationClass = "org.cthing.gradle.plugins.greeting.BuildGreetingPlugin"
        })
    }
}
...
tasks {
    ...
    withType<Test> {
        ...
        // Silence the warnings about reflective access by explicitly allowing it
